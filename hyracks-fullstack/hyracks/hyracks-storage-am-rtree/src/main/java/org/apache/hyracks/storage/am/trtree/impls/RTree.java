...
package org.apache.hyracks.storage.am.trtree.impls;
...
import org.apache.hyracks.storage.am.trtree.api.IRTreeFrame;
import org.apache.hyracks.storage.am.trtree.api.IRTreeInteriorFrame;
import org.apache.hyracks.storage.am.trtree.api.IRTreeLeafFrame;
import org.apache.hyracks.storage.am.trtree.frames.RTreeNSMFrame;
import org.apache.hyracks.storage.am.trtree.frames.RTreeNSMInteriorFrame;
...
//    }
    private void update(ITupleReference tuple, RTreeOpContext ctx) throws HyracksDataException {
        ctx.reset();
        ctx.setTuple(tuple);
        ...
        if (leafNode == null) {
            throw HyracksDataException.create(ErrorCode.FAILED_TO_FIND_TUPLE, "The tuple could not be found for update.");
        }

        int tupleIndex = ctx.getLeafFrame().findTupleIndex(tuple, ctx.getCmp());
        if (tupleIndex == -1) {
            throw HyracksDataException.create(ErrorCode.FAILED_TO_FIND_TUPLE, "The tuple could not be found for update.");
        }

        // 检查元组是否还适合当前的叶节点
        boolean fitsInLeaf = ctx.getLeafFrame().checkIfFits(tuple, ctx.getCmp());
        ...
        if (!fitsInLeaf) {
            ...
            // 找到合适的节点并插入新元组
            ICachedPage newNode = findLeaf(ctx); // 重新寻找合适的节点，因为MBR可能已变
            if (newNode == null) {
                throw HyracksDataException.create(ErrorCode.FAILED_TO_FIND_TUPLE, "No suitable node found for re-insertion.");
            }
            int newPageId = ctx.getPathList().getLastPageId(); // 假设findLeaf设置了路径列表
            boolean isLeafNode = true; // 如果是RTree的情况，这里总是叶节点
            insertTuple(newNode, newPageId, tuple, ctx, isLeafNode);
        } else {
            // 如果适合，则在原地更新元组
            ctx.getLeafFrame().update(tuple, tupleIndex, true);
            // 将更新后的页面标记为脏页
            leafNode.releaseWriteLatch(true);
        }
...
//        public void reInsert(ITupleReference tuple) throws HyracksDataException {
//            ctx.setOperation(IndexOperation.REINSERT);
//            reInsert(tuple, ctx);
//        }
        ...
        private void insert(ITupleReference tuple, IIndexOperationContext ictx) throws HyracksDataException {
            ...
            try {
                ...
                // update the node recursively from bottom to top
                while (true) {
        ...
        /**
         * Reinserts a tuple into the TR-tree.
         * This function is used to rebalance the tree after significant changes in the tuple's MBR or related metrics.
         *
         * @param tuple The tuple to be reinserted.
         * @param ctx The operational context of the TR-tree, containing all necessary state and configuration.
         * @throws HyracksDataException if any database operation fails.
         */
        private void reInsert(ITupleReference tuple, RTreeOpContext ctx) throws HyracksDataException {
            // Delete the tuple first to remove it from its current location in the tree
            delete(tuple);

            // After deleting, reinsert the tuple which may end up in the same or a different location
            insert(tuple, ctx);
        }
//
//        /**
//         * Reinserts all entries from a given node in the TR-tree.
//         * This function is used for rebalancing or reconstructing parts of the tree.
//         *
//         * @param nodePageId The page ID of the node from which entries are to be reinserted.
//         * @param ctx The operational context of the TR-tree.
//         * @throws HyracksDataException if any database operation fails.
//         */
//        private void reInsert(int nodePageId, RTreeOpContext ctx) throws HyracksDataException {
//            // Pin the node page in buffer cache to access it
//            ICachedPage nodePage = bufferCache.pin(BufferedFileHandle.getDiskPageId(getFileId(), nodePageId), false);
//            nodePage.acquireWriteLatch();
//
//            try {
//                IRTreeFrame frame = ctx.getLeafFrame(); // Assume it's a leaf frame by default
//                frame.setPage(nodePage);
//
//                if (!frame.isLeaf()) { // If it's not a leaf, use the interior frame
//                    frame = ctx.getInteriorFrame();
//                    frame.setPage(nodePage);
//                }
//
//                // Collect all entries from the node
//                List<ITupleReference> entries = new ArrayList<>();
//                for (int i = 0; i < frame.getTupleCount(); i++) {
//                    entries.add(ctx.getTuple());
//                }
//
//                // Delete all entries and then reinsert them
//                for (ITupleReference entry : entries) {
//                    delete(entry);  // Call delete method
//                    insert(entry); // Call insert method
//                }
//            } finally {
    ...
    public class RTreeBulkLoader extends AbstractTreeIndexBulkLoader {
        ...
        protected void finish() throws HyracksDataException {
            ...
            for (NodeFrontier n : nodeFrontiers) {
                ...
                //set next guide MBR
                //if propagateBulk didn't have to do anything this may be un-necessary
