...
import org.apache.hyracks.storage.am.trtree.impls.PathList;
...
//    }
    ...
    @Override
    public boolean checkIfEnlarementIsNeeded(ITupleReference tuple, MultiComparator cmp) throws HyracksDataException {
        if (RTreeComputationUtils.containsRegion(frameTuple, tuple, cmp, keyValueProviders)) {
            return false;  // 如果已包含，不需要扩大
        } else {
            // 计算如果包含新元素所需的扩大量
            double enlargement = RTreeComputationUtils.calculateExpansionRatio(frameTuple, tuple, cmp, keyValueProviders);
            return enlargement > 0.3;  // 与阈值比较，决定是否扩大
        }
    }
    ...
    public void insert(ITupleReference tuple, int tupleIndex) {
        ...
        System.out.println("Insert operation took: " + (endTime - startTime) + " nanoseconds.");
    }
