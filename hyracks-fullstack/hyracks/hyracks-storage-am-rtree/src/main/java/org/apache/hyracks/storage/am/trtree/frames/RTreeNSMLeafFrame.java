...
public class RTreeNSMLeafFrame extends RTreeNSMFrame implements IRTreeLeafFrame {
    ...
    @Override
    public boolean checkIfFits(ITupleReference tuple, MultiComparator cmp) throws HyracksDataException {
        // 获取元组所需的字节数，包括元组本身的字节和槽的大小。
        int tupleSize = getBytesRequiredToWriteTuple(tuple);

        // 获取页面中剩余的空间
        int freeSpace = calculateFreeSpace();

        // 如果剩余空间大于或等于元组所需的空间，则返回 true，表示可以插入元组。
        return freeSpace >= tupleSize;
    }

    private int calculateFreeSpace() {
        // 总的可用空间 = 页面大小 - 已使用的空间
        // 已使用的空间可以通过管理元组数量和元组位置来计算。
        int usedSpace = buf.getInt(Constants.FREE_SPACE_OFFSET);
        int tupleCount = buf.getInt(Constants.TUPLE_COUNT_OFFSET);
        int headerSize = Constants.RESERVED_HEADER_SIZE; // 如果有额外的头部信息，也应包括在内。
        return buf.capacity() - usedSpace - (tupleCount * slotManager.getSlotSize()) - headerSize;
    }
