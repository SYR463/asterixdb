...
package org.apache.hyracks.storage.am.trtree.frames;
...
public class RTreeComputationUtils {
     ...
     * @param newTuple
     * @param cmp
     * @param keyValueProviders
     * @return  expansionRatio -- the ratio of enlarged
     * @throws HyracksDataException
     */
    public static double calculateExpansionRatio(ITupleReference oldTuple, ITupleReference newTuple, MultiComparator cmp,
                                                 IPrimitiveValueProvider[] keyValueProviders) throws HyracksDataException {
        double oldArea = RTreeComputationUtils.area(oldTuple, cmp, keyValueProviders);
        double newArea = 1.0;

        int maxFieldPos = cmp.getKeyFieldCount() / 2;
        for (int i = 0; i < maxFieldPos; i++) {
            int j = maxFieldPos + i;
            double newHigh, newLow, oldHigh, oldLow;

            oldLow = keyValueProviders[i].getValue(oldTuple.getFieldData(i), oldTuple.getFieldStart(i));
            newLow = keyValueProviders[i].getValue(newTuple.getFieldData(i), newTuple.getFieldStart(i));
            double pLow = Math.min(oldLow, newLow);

            oldHigh = keyValueProviders[j].getValue(oldTuple.getFieldData(j), oldTuple.getFieldStart(j));
            newHigh = keyValueProviders[j].getValue(newTuple.getFieldData(j), newTuple.getFieldStart(j));
            double pHigh = Math.max(oldHigh, newHigh);

            newArea *= (pHigh - pLow);
        }

        double areaBeforeEnlarge = oldArea;
        double areaAfterEnlarge = newArea;
        double expansionRatio = (areaAfterEnlarge - areaBeforeEnlarge) / areaBeforeEnlarge;
